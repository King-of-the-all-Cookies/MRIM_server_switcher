...
void read_servers() {
    ...
    while (fgets(buffer, sizeof(buffer), file)) {
        ...
        if (strstr(line, "\"name\"")) {
            ...
            // Должна быть открывающая кавычка
            if (*start != '\"') continue;
            start++;
        ...
        // Ищем поле "address"
            ...
            // Сохраняем сервер, если у нас есть оба поля
            if (strlen(current_name) > 0 && strlen(current_address) > 0) {
                ...
                // Сбрасываем для следующего сервера
                current_name[0] = '\0';
                current_address[0] = '\0';
            }
        }
...
BOOL write_registry(const char* server_address) {
    HKEY hKey;
    LONG result;
    const char* keyPath = "Software\\Mail.Ru\\Agent";
    ...
    // Для версии 6.4 добавляем SSL deny
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, 
    ...
    // Создаем окно
    HWND hwnd = CreateWindowExA(0, CLASS_NAME, "Mail.Ru Agent Server Changer",
